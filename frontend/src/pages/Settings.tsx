...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)}
      className={cn('relative w-10 h-5 rounded-full transition-colors shrink-0', checked ? 'bg-indigo-500/70' : 'bg-white/10')}>
    ...
    <div className="w-full max-w-5xl space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-5">
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={cn(
                ...
                tab === t.id ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'text-white/45 hover:text-white/80 hover:bg-white/5 border border-transparent'
              )}>
        ...
        <div className="space-y-5 min-w-0">
          ...
          {tab === 'notifications' && (
            settingsLoading || !settings ? <div className="h-64 rounded-2xl bg-white/5 shimmer" /> :
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <input value={user.email} disabled
            className="w-full glass rounded-xl px-3 py-2.5 text-sm text-white/40 border border-white/10 cursor-not-allowed" />
        </div>
        <div className="space-y-1.5">
    ...
    <>
      ...
      <SectionCard title="Sessions & Devices" icon={Smartphone}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white/70 text-sm">Logout all other devices</p>
            <p className="text-white/35 text-xs mt-0.5">Signs out every session except this one. You'll stay logged in here.</p>
          </div>
        ...
        <div className="pt-3 border-t border-white/5 space-y-2 opacity-50">
          {['Two-factor authentication (2FA)', 'Trusted devices', 'Detailed login history'].map(f => (
            <div key={f} className="flex items-center justify-between text-xs">
              <span className="text-white/50 flex items-center gap-1.5"><History className="w-3 h-3" />{f}</span>
              <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-white/30">Coming soon</span>
            </div>
          ))}
        </div>
      </SectionCard>
    ...
    <SectionCard title="Notifications" icon={Bell}>
      {Object.keys(NOTIF_LABELS).map(key => (
        <div key={key} className="flex items-center justify-between py-1.5">
          <p className="text-white/75 text-sm">{NOTIF_LABELS[key]}</p>
...
function AIPreferencesTab({ prefs, onSave, saving }: { prefs: { provider: string; tone: string }; onSave: (p: any) => void; saving: boolean }) {
  ...
  return (
    <SectionCard title="AI Preferences" icon={Sparkles}>
      <div>
        <p className="text-xs text-white/40 uppercase tracking-wider mb-2">Preferred AI Provider</p>
        <div className="flex flex-wrap gap-2">
          {['groq', 'openrouter', 'gemini', 'claude', 'openai'].map(p => (
            <button key={p} onClick={() => setProvider(p)}
              className={cn('px-3 py-1.5 rounded-xl text-xs border capitalize',
                provider === p ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-300' : 'border-white/10 text-white/45 hover:text-white/75')}>
              {p}
            </button>
          ))}
        </div>
        <p className="text-[10px] text-white/25 mt-1.5">HireFlow currently runs on Groq under the hood — other providers are on the roadmap.</p>
      </div>
      <div>
        <p className="text-xs text-white/40 uppercase tracking-wider mb-2">Writing Tone</p>
        <div className="flex flex-wrap gap-2">
          {['formal', 'friendly', 'corporate', 'startup'].map(t => (
            <button key={t} onClick={() => setTone(t)}
              className={cn('px-3 py-1.5 rounded-xl text-xs border capitalize',
                tone === t ? 'bg-purple-500/20 border-purple-500/40 text-purple-300' : 'border-white/10 text-white/45 hover:text-white/75')}>
              {t}
            </button>
          ))}
        </div>
        <p className="text-[10px] text-white/25 mt-1.5">Used when generating application emails and cover letters.</p>
      </div>
      {dirty && (
  ...
  return (
    <SectionCard title="Apply Preferences" icon={Zap}>
      <div className="flex items-center justify-between p-3 rounded-xl bg-purple-500/8 border border-purple-500/20">
        <div>
          <p className="text-white/80 text-sm font-medium">Auto Apply</p>
          <p className="text-white/40 text-xs mt-0.5">Automatically send applications for jobs above your minimum match score</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs text-white/40 uppercase tracking-wider">Only above match score</label>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-white/70 w-10 text-right">{local.min_match_score}%</span>
          </div>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs text-white/40 uppercase tracking-wider">Daily send limit</label>
          <input type="number" min={1} max={450} value={local.daily_limit}
            onChange={e => setLocal({ ...local, daily_limit: Number(e.target.value) })}
            className="w-full glass rounded-xl px-3 py-2 text-sm text-white/70 border border-white/10 focus:border-indigo-500/40 focus:outline-none" />
        </div>
      </div>
          ...
          {['dark', 'oled', 'light'].map(t => (
            <button key={t} onClick={() => setLocal({ ...local, theme: t })}
              className={cn('flex-1 px-3 py-2.5 rounded-xl text-xs border capitalize',
                local.theme === t ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-300' : 'border-white/10 text-white/45 hover:text-white/75')}>
              {t === 'oled' ? 'OLED Black' : t}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          {[['purple', '#a855f7'], ['blue', '#6366f1'], ['green', '#10b981']].map(([name, color]) => (
            <button key={name} onClick={() => setLocal({ ...local, accent: name })}
              className={cn('w-9 h-9 rounded-xl border-2 flex items-center justify-center', local.accent === name ? 'border-white' : 'border-transparent')}
              style={{ background: color }} />
          ))}
        </div>
      </div>
      <div className="flex items-center justify-between py-1">
        <p className="text-white/70 text-sm">Animations</p>
        <Toggle checked={local.animations} onChange={v => setLocal({ ...local, animations: v })} />
      </div>
      <div className="flex items-center justify-between py-1">
        <p className="text-white/70 text-sm">Compact Mode</p>
  ...
  return (
    <>
      <SectionCard title="Your Data" icon={Globe2}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white/70 text-sm">Clear AI history</p>
            <p className="text-white/35 text-xs mt-0.5">Wipes cached career insights and job AI analyses. Scores and matches are unaffected.</p>
          </div>
          <button onClick={() => clearAi.mutate()} disabled={clearAi.isPending}
            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/10 text-white/60 text-xs hover:text-white hover:border-white/25 disabled:opacity-50 shrink-0">
            {clearAi.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />} Clear
          </button>
        </div>
        <div className="flex items-center justify-between pt-3 border-t border-white/5">
          <div>
            <p className="text-white/70 text-sm">Download your data</p>
            <p className="text-white/35 text-xs mt-0.5">Export every job, resume, application and activity record as JSON</p>
          </div>
          <button onClick={downloadData}
            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/10 text-white/60 text-xs hover:text-white hover:border-white/25 shrink-0">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl border border-red-500/20 p-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/35 text-xs mt-0.5">Permanently delete your account and all associated data. This cannot be undone.</p>
          </div>
        ...
        {confirmOpen && (
          <div className="mt-4 pt-4 border-t border-red-500/15 space-y-3">
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Your password"
              className="w-full max-w-xs glass rounded-xl px-3 py-2 text-sm text-white/70 placeholder:text-white/25 border border-red-500/25 focus:outline-none" />
            <div className="flex gap-2">
              ...
              <button onClick={() => { setConfirmOpen(false); setPassword('') }} className="px-4 py-2 rounded-xl border border-white/10 text-white/50 text-xs hover:text-white">
                Cancel
              </button>
            </div>
          </div>
