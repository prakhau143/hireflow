...
export default function AIInsights({ className, delay = 0 }: { className?: string; delay?: number }) {
  ...
  return (
    <motion.div
      ...
      className={cn('glass rounded-2xl border border-white/10 p-5 relative overflow-hidden', className)}
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-white/35 text-xs mt-0.5">Hiring trends & personalized recommendations</p>
          </div>
        </div>
        <button
          onClick={() => (data ? forceRefresh() : refetch())}
          disabled={isFetching}
          className="p-2 rounded-lg glass border border-white/10 text-white/40 hover:text-white/80 hover:border-white/25 transition-all disabled:opacity-40"
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-14 rounded-xl bg-white/[0.03] shimmer" />
          ))}
        </div>
      ) : !data?.insights?.length ? (
        <p className="text-white/25 text-sm py-8 text-center">No Data Available</p>
      ) : (
          ...
          {data.insights.map((insight, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all"
              >
                <div className={cn('w-7 h-7 rounded-lg border flex items-center justify-center shrink-0 mt-0.5', style.chip)}>
                  <Icon className="w-3.5 h-3.5" />
                </div>
                <div className="min-w-0">
                  <p className="text-white/85 text-sm font-medium leading-snug">{insight.title}</p>
                  <p className="text-white/40 text-xs mt-1 leading-relaxed">{insight.detail}</p>
                </div>
              </motion.div>
            )
          })}
